...
import numpy as np
from model_ADNI import PiCO_ADNI
from GCN import *
from utils.utils_algo import *
from utils.utils_loss import partial_loss, SupConLoss
from utils.ADNI import load_ADNI
...
parser = argparse.ArgumentParser(description='PyTorch implementation of PLL for ADNI dataset')
parser.add_argument("--local_rank", type=int, default=0)
parser.add_argument('--dataset', default='ADNI', type=str,
                    choices=['cifar10', 'cifar100', 'cub200', 'ADNI'],
                    help='dataset name (cifar10)')
parser.add_argument('--exp-dir', default='experiment/ADNI', type=str,
                    help='experiment directory for saving checkpoints and logs')
parser.add_argument('-a', '--arch', metavar='ARCH', default='GCN', choices=['resnet18', 'GCN'],
                    help='network architecture (GCN i used for ADNI dataset)')
...
def main_worker(gpu, ngpus_per_node, args):
    ...
    if args.dataset == 'ADNI':
        train_loader, train_givenY, train_sampler, test_loader = load_ADNI(partial_rate=args.partial_rate, batch_size=args.batch_size)
    else:
...
def train(train_loader, model, loss_fn, loss_cont_fn, optimizer, epoch, args, tb_logger, start_upd_prot=False):
    ...
    for i, (orig_feat, orig_adj, aug_feat, aug_adj, labels, true_labels, index) in enumerate(train_loader):
        # measure data loading time
        data_time.update(time.time() - end)

        f_w, adj_w, f_s, adj_s, Y, index = orig_feat.cuda(), orig_adj.cuda(), aug_feat.cuda(),aug_adj.cuda(), labels.cuda(), index.cuda()
...
### add adj~!!!
        cls_out, features_cont, pseudo_score_cont, partial_target_cont, score_prot\
            = model(f_w, adj_w, f_s, adj_s, Y, args)
...
def test(model, test_loader, args, epoch, tb_logger):
    with torch.no_grad():
        ...
        for batch_idx, (feature, adj, labels) in enumerate(test_loader):
            feature, adj, labels = feature.cuda(), adj.cuda(), labels.cuda()
            outputs = model(feature, adj, im_k=None, adj_k=None, args=args, eval_only=True)
            acc1, acc2 = accuracy(outputs, labels, topk=(1, 2))
            top1_acc.update(acc1[0])
            top2_acc.update(acc2[0])
        
        # average across all processes
        acc_tensors = torch.Tensor([top1_acc.avg,top2_acc.avg]).cuda(args.gpu)
        ...
        if args.gpu ==0:
            ...
            tb_logger.log_value('Top2 Acc', acc_tensors[1], epoch)
