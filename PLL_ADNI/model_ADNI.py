...
class PiCO_ADNI(nn.Module):

    def __init__(self, args, base_encoder):
        super().__init__()
        

        self.encoder_q = base_encoder(out_dim=args.num_class, pred_dim1=args.low_dim)
        # momentum encoder
        self.encoder_k = base_encoder(out_dim=args.num_class, pred_dim1=args.low_dim)
...
# adj_q, adj_k!!!
    def forward(self, img_q, adj_q, im_k, adj_k, partial_Y=None, args=None, eval_only=False):
        output, q = self.encoder_q(img_q, adj_q)
        if eval_only:
            return output
        # for testing
