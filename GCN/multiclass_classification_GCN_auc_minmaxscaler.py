...
list_score = graph_label_csv.to_numpy()

### (3) edge feature threshold + scaling (이미 numpy였음)

##### 택1 할 것 (3-1) threshold as 20
...
##### 택1 할 것 (3-2) threshold as 20 & make it symmetric
for i in range(list_adj.shape[0]):
    for j in range(list_adj.shape[1]):
        for k in range(list_adj.shape[2]):
            if list_adj[i][j][k] <=20:
                ...
                pass
            if j<k: #다 0으로 assign되어있음          
                list_adj[i][j][k] = list_adj[i][k][j] #make it symmetric

##### (3-3) scaling with minmax scaler
...
class GCNDataset(Dataset):
    def __init__(self, list_feature, list_adj, list_score):
    ...
    def __getitem__(self, index):
        return self.list_feature[index], self.list_adj[index], self.list_score[index]
    
    
def partition(list_feature, list_adj, list_score, args):
    ...
    score_train = list_score[:num_train]
    feature_val = list_feature[num_train:num_train + num_val]
    adj_val = list_adj[num_train:num_train + num_val]
    score_val = list_score[num_train:num_train + num_val]
    feature_test = list_feature[num_total - num_test:]
    adj_test = list_adj[num_total - num_test:]
    score_test = list_score[num_total - num_test:]
    
    train_set = GCNDataset(feature_train, adj_train, score_train)
    val_set = GCNDataset(feature_val, adj_val, score_val)
    test_set = GCNDataset(feature_test, adj_test, score_test)
...
def multi_acc(y_pred, y_test):
    ...
    print('shape of y_test:', y_test.shape) #0, 1, 2로 되어있음!
    y_pred_softmax = torch.log_softmax(y_pred, dim=0)
    print('shape of y_pred_softmax:', y_pred_softmax.shape)
...
def multi_auc(y_pred, y_test):
    y_pred_softmax = nn.Softmax(y_pred)
    print('shape of y_pred_softmax:' , y_pred_softmax.shape)
    print('shape of y_test:' , y_test.shape)
    
    auc = roc_auc_score(y_score=y_preds_softmax, y_true=y_test.numpy(), multi_class='ovr')
...
def train(model, device, optimizer, criterion, data_train, bar, args):
    ...
    for i, batch in enumerate(data_train):
        ...
        list_score = batch[2].clone().to(device).float().detach().requires_grad_(True)
        list_score = list_score.view(-1, 1)
        list_score = list_score.type(torch.long)
        #print('shape of list_score is:', list_score.shape)
        #print('list_score is:', list_score)
        
        model.train()
        optimizer.zero_grad()
        list_pred_score = model(list_feature, list_adj)
        #print('shape of list_pred_score is:', list_pred_score.shape)

        list_pred_score.require_grad = False
        train_loss = criterion(list_pred_score, list_score.squeeze(dim=-1))
...
def validate(model, device, criterion, data_val, bar, args):
    ...
    with torch.no_grad():
        for i, batch in enumerate(data_val):
            ...
            list_pred_score = model(list_feature, list_adj)
            list_pred_score.require_grad = False
            val_loss = criterion(list_pred_score, list_score.squeeze(dim=-1))
...
def test(model, device, data_test, args):
    ...
    with torch.no_grad():
        score_total = list()
        pred_score_total = list()
        for i, batch in enumerate(data_test):
            list_feature = batch[0].clone().to(device).float().detach().requires_grad_(True)
            list_adj = batch[1].clone().to(device).float().detach().requires_grad_(True)
            #원형은... torch.tensor(batch[1]).to(device).float()
            list_score = batch[2].clone().to(device).float().detach().requires_grad_(True)
            score_total += list_score.tolist()
            list_score = list_score.view(-1,1)
            list_score = list_score.type(torch.long)

            list_pred_score = model(list_feature, list_adj)
            pred_score_total += list_pred_score.view(-1).tolist()

        print('true label: ', score_total)
        print('pred label: ', pred_score_total)
        #s = nn.Softmax()
        #preds_prob = s(pred_score_total)
        #print('pred probability: ', pred_score_total)
        acc = multi_acc(pred_score_total, score_total)
        #auc = multi_auc(pred_score_total, score_total)
        #std = np.std(np.array(score_total)-np.array(pred_score_total))

    return acc, score_total, pred_score_total
...
def experiment(dict_partition, device, bar, args):
    ...
    acc, score_total, pred_score_total = test(model, device, data_test, args) #원래 auc 가 앞에 있었음.
    ...
    args.score_total = score_total
    args.pred_score_total = pred_score_total
    args.acc = acc
    args.auc = auc
...
dict_partition = partition(list_feature, list_adj, list_score, args)
...
# 4-2. set device
device = torch.device('cuda:2' if torch.cuda.is_available() else 'cpu') #여기 바꿔가면서 실험해야 함
...
for hy in h_space:
    ...
    print(args.exp_name + " took " + str(int(args.time_required)) + "seconds.") # + " auc is: " + str(args.auc) + " acc is: " + str(args.acc))
...
df_result.to_csv('./GCN_result/GCN_hyp_tuning_ADNI_minmax_scaled_metric_auc_asymm_mat.csv')