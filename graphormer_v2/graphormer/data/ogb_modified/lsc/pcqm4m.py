...
class PCQM4MEvaluator:
    ...
    def eval(self, input_dict):
        ...
        if isinstance(y_true, torch.Tensor):
            criterion = RMSELoss()
            return {'rmse': criterion(y_true, y_pred).cpu().item()}
        else:
            return {'rmse': float(np.sqrt(mean_squared_error(y_pred, y_true)))}
    ...
    def save_test_submission(self, input_dict: Dict, dir_path: str):
        ...
        filename = osp.join(dir_path, 'y_pred_abcd') #_pcqm4m')
...
#     dataset = PCQM4MDataset(only_smiles=False) #True)
#     print(dataset)
#     print(dataset[1234])
