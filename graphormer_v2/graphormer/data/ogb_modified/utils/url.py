...
def decide_download(url):
    d = ur.urlopen(url)
    size = 0.5 #= int(d.info()["Content-Length"])/GBFACTOR
...
def download_url(url, folder, log=True):
    ...
    print("path is: ", path)
    log=True
    osp.exists(path) = True
    osp.getsize(path) = 1

    #if osp.exists(path) and osp.getsize(path) > 0:  # pragma: no cover
    if log:
        print('Using exist file', filename) #이렇게 하도록 조작을 가해야 함.
    return path
    ...
    size = 0.5 #= int(data.info()["Content-Length"])
...
def extract_tar(path, folder, log=True):
    '''
    extract a tar archive to a specific folder.
    '''
    maybe_log(path, log)
