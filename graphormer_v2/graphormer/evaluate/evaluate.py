...
sys.path.append( path.dirname( path.dirname( path.abspath(__file__) ) ) ) #graphormer directory에 대한 접근 권한 겟!
from data import ogb_modified
from data.ogb_modified.lsc import pcqm4m
...
def eval(args, use_pretrained, checkpoint_path=None, logger=None):
    ...
    if use_pretrained:
        if cfg.task.pretrained_model_name == "pcqm4mv1_graphormer_base":
            ...
            evaluator = ogb_modified.lsc.PCQM4Mv2Evaluator()
            input_dict = {'y_pred': y_pred, 'y_true': y_true}
            result_dict = evaluator.eval(input_dict)
            logger.info(f'PCQM4Mv2Evaluator: {result_dict}')
        else: # our abcd models
            evaluator = pcqm4m.PCQM4MEvaluator()
            input_dict = {'y_pred': y_pred, 'y_true': y_true}
            result_dict = evaluator.eval(input_dict)
            logger.info(f'ABCD_Evaluator: {result_dict}')
    else:
        if args.metric == "auc":
