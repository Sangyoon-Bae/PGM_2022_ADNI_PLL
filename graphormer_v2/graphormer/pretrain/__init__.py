...
def load_pretrained_model(pretrained_model_name):
    pretrained_model = torch.load(pretrained_model_name)['model']
...
#     else:
#     load_state_dict_from_url(PRETRAINED_MODEL_URLS[pretrained_model_name], progress=True, file_name=f"{pretrained_model_name}_{dist.get_rank()}")["model"]
#         dist.barrier()